    ...
    public static boolean createAnswerCardImage(String path, String desFile, String title, List<QuestionItem> questionItems) throws Exception {
        ...
        for (int i = 0; i < rowNum; i++) {
            ...
            for (int j = 0; j < 20; j++) {
                ...
                Imgproc.putText(img, "" + (i * 20 + j + 1), new Point(x, y), Imgproc.FONT_HERSHEY_SCRIPT_COMPLEX, 1, new Scalar(0, 0, 0));
                ...
                for (int z = 1; z <= 4; z++) {
                    ...
                    Imgproc.putText(img, option, new Point(x, optionY), Imgproc.FONT_HERSHEY_DUPLEX, 1, new Scalar(0, 0, 0));
                    optionY += 100;
                }
                x += 100;
                if ((j + 1) % 4 == 0)
